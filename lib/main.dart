...
void main() =>
    runApp(CazinoApp(controller: AppController(LocalDemoRepository())));
...
class _RegisterState extends State<RegisterScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(20), children: [
        const Text('Use your real personal details',
            style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
        const Text(
            'This demo keeps data in memory only. Production identity data must be encrypted and handled by an approved KYC provider.'),
        const SizedBox(height: 16),
            ...
            onPressed: () async {
              ...
              if (context.mounted)
                Navigator.pushReplacement(
                    ...
                    MaterialPageRoute(
                        builder: (_) =>
                            FicaScreen(controller: widget.controller)));
            },
            child: const Text('Continue to FICA')),
      ]));
}
  ...
  Widget build(BuildContext context) => Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(20), children: [
        ...
        _MenuCard(
            ...
            subtitle: 'Demo document staging workflow',
            onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (_) => FicaScreen(controller: controller)))),
      ]));
}
...
class LobbyScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final players =
        controller.repository.players.where((p) => p.id != controller.user!.id);
    return Scaffold(
            ...
            children: players
                .map((p) => ListTile(
                    ...
                    trailing: FilledButton.tonal(
                        onPressed: () async {
                          await controller.challenge(p.id);
                          if (context.mounted)
            ...
            : ListView(
                children: items
                    .map((c) => ListTile(
                        ...
                        subtitle: Text(c.fromPlayerId == controller.user!.id
                            ? 'Sent'
                            : 'Received')))
                    .toList()));
  }
      ...
      body: ListView(padding: const EdgeInsets.all(20), children: [
        ...
        FilledButton(
            onPressed: docs.length == 3
                ? () => ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                        content: Text('Demo FICA submission recorded')))
                : null,
...
class _GameState extends State<GameScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final proposedHigherTarget =
        actionBuild == null ? looseAdded : actionBuild.target + looseAdded;
    final retainsHigherTarget = proposedHigherTarget <= 10 &&
        view.hand.any(
            (card) => card != selected && card.rank == proposedHigherTarget);
    final constructTotal = editingSimpleBuild && retainsHigherTarget
        ? proposedHigherTarget
        : actionBuild != null
            ? actionBuild.target
            : retainsRawTarget
                ? looseAdded
    ...
    return Scaffold(
        appBar: AppBar(
            title: Text(
                '$activeName • Round ${view.phase == GamePhase.roundOne ? 1 : 2}'),
            actions: [
              IconButton(
